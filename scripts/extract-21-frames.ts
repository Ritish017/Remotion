...
async function extract21Frames() {
  const videoPath = path.join(process.cwd(), 'PHASE4_SHOWCASE.mp4');
  const outputDir = path.join(process.cwd(), 'storage', 'qa', 'phase4_human_review');
  ...
  console.log('Extraction of 21 frames completed successfully.');
}

extract21Frames().catch(err => {
