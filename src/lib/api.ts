...
export const getJobStatus = (id: string) => fetch(`${BASE}/status/${id}`).then(r => r.json())
...
// Library
export const getLibraryFamilies = () => fetch(`${BASE}/library/families`).then(r => r.json())
export const getScenes = (type?: string) => fetch(`${BASE}/scenes${type ? `?scene_type=${type}` : ''}`).then(r => r.json())
